...
public class LooksRemoteDataSource {
    
    public init() {}
}

extension LooksRemoteDataSource: AppData.LooksRemoteDataSource {

    public func getLooks(completion: @escaping (Result<LooksModel, Error>) -> ()) {
        #warning("TODO: connect with real data source")
        ...
        let JSON = """
        {
         "looks": [
           ...
           {
             "id": 3,
             "song_url": "https://audio-ssl.itunes.apple.com/itunes-assets/AudioPreview122/v4/45/0d/c6/450dc66a-d6f5-5bdd-19d7-3ba3c0a6f9fb/mzaf_8198542298496411575.plus.aac.p.m4a",
           ...
           {
             "id": 4,
             "song_url": "https://audio-ssl.itunes.apple.com/itunes-assets/AudioPreview112/v4/54/be/0c/54be0ce0-475d-eb5e-9464-4c2abc3b0a0e/mzaf_7215179913584518329.plus.aac.p.m4a",
    ...
    """
        ...
        guard let jsonData = JSON.data(using: .utf8) else {
            completion(.failure(LooksErrors.unexpectedError))
            return
        }
        DecodeData.decodeFor(LooksModel.self, data: jsonData) { result in
            switch result {
            case .success(let response):
                completion(.success(response))
            case .failure:
                completion(.failure(LooksErrors.decodeError))
            }
        }
    }
}

enum LooksErrors: Error {
